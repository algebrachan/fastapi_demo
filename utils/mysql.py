...
from models.demo_model import Base
import time
...
# 操作mysql数据库 demo
#  data = session.query(DetectionResult).filter(and_(DetectionResult.date <= date, DetectionResult.type == type)).order_by(desc(DetectionResult.date)).first()
